...
read_json_values("settings_apollo_3.json")
...
def main():
    ...
    try:
        input_file = os.getcwd()+r"/bot_use_only/input_3.csv"
    ...
    for x in range(last_processed_index, len(df)):
        ...
        if run_code:
            ...
            update_last_processed_page("settings_apollo_3.json",last_processed_page)
            for page in range(last_processed_page,99):
                ...
                if len(all_links)==0:
                    ...
                    last_processed_person = -1
                    update_last_processed_page("settings_apollo_3.json", last_processed_page)
                    update_last_processed_person("settings_apollo_3.json", last_processed_person)
                    break
                for count,lead in enumerate(all_links,last_processed_person+1):
                    ...
                    csv_file_path = f"{os.getcwd()}/bot_use_only/output_apollo_3.csv"
                    ...
                    csv_file_path = f"{os.getcwd()}/bot_use_only/output_apollo_company_3.csv"
            ...
            last_processed_page = 1
            last_processed_person = -1
            update_last_processed_index("settings_apollo_3.json",last_processed_index)
            update_last_processed_page("settings_apollo_3.json",last_processed_page)
            update_last_processed_person("settings_apollo_3.json",last_processed_person)
    ...
    if last_processed_index == len(df):
        ...
        update_last_processed_index("settings_apollo_3.json",0)
        update_last_processed_person("settings_apollo_3.json",-1)
        update_last_processed_page("settings_apollo_3.json", 1)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if is_weekday() == True:
            if datetime_time(9, 15) <= current_time < datetime_time(18, 15):
                if stop_code() == True:
                    continue
                print("Starting Code..")
                start_code()
        else:
            if datetime_time(9, 15) <= current_time < datetime_time(16, 15):
                if stop_code() == True:
                    continue
                print("Starting code...")
                start_code()
